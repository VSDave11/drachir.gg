...
// Validace nové žádosti: vlastní směna (manažer smí nabídnout jakoukoli), ne RIP/Vacation.
function validateNewRequest(shift, userName, isManager) {
    if (!userName) return 'Missing user';
    if (!shift || !shift.date || !shift.product || !shift.start) return 'Invalid shift';
    if (!isManager && shift.name && shift.name !== userName) return 'You can only offer your own shift';
    if (shift.product === 'Vacation' || shift.product === 'RIP') return 'This item cannot be swapped';
    return null;
}
