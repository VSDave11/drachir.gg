...
function validatePersonInput(person, opts) {
    ...
    if (!groups.includes(group)) return 'Unknown group: ' + group;
    if (mode === 'add' && existingNames.includes(name)) return 'Person "' + name + '" already exists';
    if (mode === 'update' && !existingNames.includes(name)) return 'Person "' + name + '" does not exist';
    return null;
}
